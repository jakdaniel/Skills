...
2. **Is the sink reachable with that input?** Check whether validation, an allowlist, an ORM,
   ...
   (`app.UseAuthorization`, `[Authorize]` attributes, a base controller, endpoint filters,
   policy handlers) before flagging a route as missing authorization — enforcement is often
...
```csharp
// UNSAFE — string concatenation (or FromSqlRaw with interpolation)
var sql = $"SELECT * FROM Users WHERE Id = {userId}";
db.Users.FromSqlRaw(sql);

// SAFE — LINQ (parameterized by EF Core) or FromSqlInterpolated
var user = await db.Users.FirstOrDefaultAsync(u => u.Id == userId, ct);
// or
var users = db.Users.FromSqlInterpolated($"SELECT * FROM Users WHERE Id = {userId}");
```

### Command Injection Prevention
```csharp
// UNSAFE — shell-interpreted string
Process.Start("cmd", $"/c convert {filename} output.png");

// SAFE — argument list, no shell
Process.Start(new ProcessStartInfo("convert", $"{filename} output.png")
{
...
// SAFE — ASP.NET Core Identity's password hasher (PBKDF2)
...
app.MapGet("/api/user/{userId:guid}", (Guid userId, UserService users) =>
    users.Get(userId));

// SAFE - Authorization enforced + ownership verified
app.MapGet("/api/user/{userId:guid}", [Authorize] async (
    Guid userId, ClaimsPrincipal principal, UserService users) =>
{
    if (!principal.HasClaim(ClaimTypes.NameIdentifier, userId.ToString()) &&
        !principal.IsInRole("admin"))
        return Results.Forbid();
    return Results.Ok(await users.Get(userId));
});
```
...
// SAFE - Fail-closed, log context (IExceptionHandler)
public sealed class GlobalExceptionHandler(ILogger<GlobalExceptionHandler> logger) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext ctx, Exception ex, CancellationToken ct)
    {
        ...
        await ctx.Response.WriteAsJsonAsync(
            new { error = "An error occurred", id = errorId }, ct);
        return true;
    }
}
```
...
```csharp
// UNSAFE - Fail-open
bool CheckPermission(User user, Resource resource)
{
    try { return _authService.Check(user, resource); }
    catch { return true; }  // DANGEROUS!
}
...
bool CheckPermission(User user, Resource resource)
{
    ...
    {
        _logger.LogError(e, "Auth check failed");
...
var prompt = $"You are a support agent. Answer this: {userInput}";
var response = await llm.CompleteAsync(prompt, ct);
...
var sql = await llm.CompleteAsync("Write a query for: " + userRequest, ct);
await db.Database.ExecuteSqlRawAsync(sql, ct);

// SAFE - constrain output, validate, and use parameterized execution
var spec = await llm.CompleteJsonAsync<QuerySpec>(userRequest, ct); // structured output
var (query, parameters) = QueryBuilder.Build(spec);                 // allow-listed columns/ops
await db.Database.ExecuteSqlRawAsync(query, parameters, ct);
```
