...
L'orchestrateur délègue aux 5 compétences du dossier :

| Compétence | Fichier | Rôle & Posture de Review |
| :--- | :--- | :--- |
| **`code-review`** | [`./code-review/SKILL.md`](./code-review/SKILL.md) | **Revue Structurée & Actionnable (Expert Front-End)**<br>• Analyse sur 5 axes : découpage des composants, SOLID front-end, Clean Architecture, FSA, bonnes pratiques Svelte.<br>• Chaque problème expliqué (le pourquoi) + solution concrète avec exemple de code corrigé.<br>• Posture pédagogique : constructive, orientée amélioration. |
...
| **`review-senior`** | [`./review-senior/SKILL.md`](./review-senior/SKILL.md) | **Revue Sénior Exigeante & Constructive (Validation Prod)**<br>• Audit rigoureux, incisif et sans concession, mais constructif (sans la toxicité extrême).<br>• Tolérance zéro sur les points bloquants : sécurité multi-tenant, `any`, fuites Edge, limites de lignes.<br>• Posture de référence pour valider un code prêt pour la production avant merge. |
| **`bug-finder`** | [`./bug-finder/SKILL.md`](./bug-finder/SKILL.md) | **Senior Bug Hunter — Traque de Bugs & Hypothèses Implicites**<br>• Analyse le code "comme s'il était déjà en production" : bugs existants, latents et futurs probables.<br>• Principe : un bug est souvent une hypothèse implicite qui n'est pas toujours vraie.<br>• Cas limites, race conditions, comportements inattendus non détectés par les tests. |

> ⚠️ **Note de versionnage** : des variantes plus riches existent ailleurs —
> `agent-code/bug-finder/` (version enrichie de la traque de bugs, usage développement) et le `code-review-extreme` global (version étendue).
> Ce dossier conserve les versions courtes ; privilégier les versions enrichies si disponibles.
...
flowchart TD
    ...
    B -->|Améliorer / Apprendre / Refactoring| C[Activer code-review]
    ...
    G --> H5[bug-finder -> code-review-sceptique -> code-review]
...
* **Compétence Principale** : [`code-review`](./code-review/SKILL.md)
* **Consignes** :
  1. Couvrir les 5 axes : découpage, SOLID, Clean Architecture, FSA, Svelte.
...
* **Compétence Complémentaire** : [`bug-finder`](./bug-finder/SKILL.md)
* **Consignes** :
...
* **Compétence Principale** : [`bug-finder`](./bug-finder/SKILL.md)
* **Consignes** :
...
* **Consignes** :
  ...
  2. Tolérance zéro sur les bloquants : isolation multi-tenant (`compagnie_id`), typage `any`, fuites Edge, limites de lignes dépassées.
...
* **Pipeline** : [`bug-finder`](./bug-finder/SKILL.md) → [`code-review-sceptique`](./code-review-sceptique/SKILL.md) → [`code-review`](./code-review/SKILL.md)
* **Consignes** :
  ...
  3. Phase 3 (code-review) : fournir les solutions concrètes et le code corrigé.

---

## 📜 Invariants Transverses à Toutes les Revues

Quel que soit le sous-skill activé, les constats s'exécutent sous les standards KalySync :
...
3. **Standards KalySync Non Négociables** :
   - Isolation multi-tenant (`WHERE compagnie_id = ?`), Runes Svelte 5 exclusives, composants < 400 lignes, zéro `any`, zéro `placeholder`, tokens CSS de `src/app.css`.
...
```
SI la demande concerne "review le code / améliore ce composant / refactoring conseillé" :
   ➜ Charger code-review/SKILL.md

SI la demande concerne "valide ce code avant merge / review de PR / est-ce risqué ?" :
   ➜ Charger code-review-sceptique/SKILL.md (+ bug-finder/SKILL.md si logique complexe)
...
SI la demande concerne "audit complet du code" :
   ➜ Pipeline : bug-finder -> code-review-sceptique -> code-review
```
...
- [ ] Les standards KalySync (compagnie_id, Runes, 400L, tokens CSS) ont-ils été vérifiés ?
